...
export function MusicPlayer({
  ...
  const pct = Math.min(100, (progress / song.seconds) * 100);
  return (
    <div className="animate-rise rounded-[18px] border border-poster-fg/20 bg-poster-shade/45 p-3 shadow-glass backdrop-blur-2xl">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate font-serif text-[0.88rem] text-poster-fg">{song.title}</p>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <span className="relative h-[3px] flex-1 overflow-hidden rounded-full bg-poster-fg/20">
          <span
            className="absolute inset-y-0 left-0 rounded-full bg-yt-red transition-[width] duration-1000 ease-linear"
            style={{ width: `${pct}%` }}
          />
        </span>
        <span className="text-[0.6rem] tabular-nums text-poster-fg/55">{song.duration}</span>
